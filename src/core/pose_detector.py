            ...
            for scale_info in scales:
                ...
                for y in range(size):
                    for x in range(size):
                        ...
                        for i in range(17):
                            idx = i * 3
                            # Keypoints são offset do centro da célula
                            kp_x = kpts_data[idx] * 2 * stride + cx
                            kp_y = kpts_data[idx + 1] * 2 * stride + cy
                            kp_conf = kpts_data[idx + 2]

                            # Sigmoid na confiança se necessário
                            if kp_conf < 0:
                                kp_conf = 1 / (1 + np.exp(-np.clip(kp_conf, -10, 10)))
